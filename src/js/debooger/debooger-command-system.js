    ...
    registerBuiltInCommands() {
        ...
        this.registerCommand('clearinventory', '🎒 PURGE inventory - void everything you own 💀', () => {
            if (typeof game !== 'undefined' && game.player) {
                ...
                if (typeof PlayerStateManager !== 'undefined' && PlayerStateManager.inventory?.clear) {
                    PlayerStateManager.inventory.clear('debug_clear');
                } else {
                    game.player.inventory = { gold: game.player.gold || 0 };
                    // Emit event for UI updates
                    if (typeof EventBus !== 'undefined') {
                        EventBus.emit('inventory:cleared', { reason: 'debug_clear' });
                    }
                }
        ...
        this.registerCommand('teleport', '🗺️ Teleport: teleport <locationId> - fold space like paper 🖤', (args) => {
            ...
            } else if (typeof WorldStateManager !== 'undefined' && WorldStateManager.setCurrentLocation) {
                // Get full location data if available
                const locData = (typeof GameWorld !== 'undefined' && GameWorld.locations?.[locationId])
                    ? { ...GameWorld.locations[locationId] }
                    : { id: locationId, name: locationId };
                WorldStateManager.setCurrentLocation(locData, 'debug_teleport');
                console.log(`🗺️ Teleported to ${locationId} via WorldStateManager`);
        ...
        this.registerCommand('doom', 'Enter DOOM WORLD: transport to random dungeon, full apocalypse mode', () => {
            ...
            const doomLocationData = {
                ...entryLocation,
                name: doomLocName,
                originalName: entryLocation.name
            };
            if (typeof WorldStateManager !== 'undefined' && WorldStateManager.setCurrentLocation) {
                WorldStateManager.setCurrentLocation(doomLocationData, 'debug_doom_enter');
                WorldStateManager.setCurrentWorld('doom', 'debug_doom_enter');
                console.log('💀 game.currentLocation set via WorldStateManager:', doomLocationData.name, doomLocationData.id);
            } else if (typeof game !== 'undefined') {
                game.currentLocation = doomLocationData;
            ...
            // 🖤 Step 2.5: UNLOCK ALL GATEHOUSES - Doom world has no toll booths!
            if (typeof GatehouseSystem !== 'undefined' && GatehouseSystem.unlockAllGates) {
                GatehouseSystem.unlockAllGates('doom_world_entry');
                results.push('GatehouseSystem: all gates unlocked');
                console.log('💀 All gatehouses unlocked for doom world');
            }
            ...
            if (typeof EventBus !== 'undefined') {
                EventBus.emit('world:changed', { world: 'doom', entryLocation: entryDungeon });
        ...
        this.registerCommand('exitdoom', 'Exit DOOM WORLD: return to normal world at current location', () => {
            if (typeof TravelSystem !== 'undefined' && TravelSystem.isInDoomWorld()) {
                ...
                if (normalLocation) {
                    if (typeof WorldStateManager !== 'undefined' && WorldStateManager.setCurrentLocation) {
                        WorldStateManager.setCurrentLocation({ ...normalLocation }, 'debug_doom_exit');
                        WorldStateManager.setCurrentWorld('normal', 'debug_doom_exit');
                        console.log('🌅 game.currentLocation set via WorldStateManager:', normalLocation.name, normalLocation.id);
                    } else if (typeof game !== 'undefined') {
                ...
                // 🖤💀 Emit world change event for any listeners
                if (typeof EventBus !== 'undefined') {
                    EventBus.emit('world:changed', { world: 'normal', exitLocation: currentLocationId });
        ...
        // ═══════════════════════════════════════════════════════════════
        // 🔍 STATE VALIDATION - check system integrity
        // ═══════════════════════════════════════════════════════════════

        // validate - Run full state validation
        this.registerCommand('validate', 'Validate all state managers and detect inconsistencies', () => {
            console.log('═══════════════════════════════════════════════════');
            console.log('🔍 STATE VALIDATION REPORT');
            console.log('═══════════════════════════════════════════════════');

            let issues = 0;

            // Check PlayerStateManager sync
            if (typeof PlayerStateManager !== 'undefined' && typeof game !== 'undefined' && game.player) {
                console.log('\n📦 PlayerStateManager:');
                const psmGold = PlayerStateManager.gold?.get() || 0;
                const gameGold = game.player.gold || 0;
                if (psmGold !== gameGold) {
                    console.warn(`  ❌ Gold mismatch: PSM=${psmGold}, game=${gameGold}`);
                    issues++;
                } else {
                    console.log(`  ✅ Gold synced: ${psmGold}`);
                }
            } else {
                console.log('📦 PlayerStateManager: Not loaded');
            }

            // Check WorldStateManager sync
            if (typeof WorldStateManager !== 'undefined') {
                console.log('\n🌍 WorldStateManager:');
                const wsmLoc = WorldStateManager.getCurrentLocationId();
                const gameLoc = game?.currentLocation?.id;
                if (wsmLoc !== gameLoc) {
                    console.warn(`  ❌ Location mismatch: WSM=${wsmLoc}, game=${gameLoc}`);
                    issues++;
                } else {
                    console.log(`  ✅ Location synced: ${wsmLoc}`);
                }

                const wsmWorld = WorldStateManager.getCurrentWorld();
                const tsWorld = typeof TravelSystem !== 'undefined' ? TravelSystem.currentWorld : null;
                if (tsWorld && wsmWorld !== tsWorld) {
                    console.warn(`  ❌ World mismatch: WSM=${wsmWorld}, TS=${tsWorld}`);
                    issues++;
                } else {
                    console.log(`  ✅ World synced: ${wsmWorld}`);
                }
            } else {
                console.log('🌍 WorldStateManager: Not loaded');
            }

            // Check UIStateManager
            if (typeof UIStateManager !== 'undefined') {
                console.log('\n🖥️ UIStateManager:');
                console.log(`  ✅ Initialized: ${UIStateManager._initialized}`);
                console.log(`  ✅ Open panels: ${Object.keys(UIStateManager._panelStates || {}).filter(k => UIStateManager._panelStates[k]).length}`);
            } else {
                console.log('🖥️ UIStateManager: Not loaded');
            }

            // Check GoldManager sync
            if (typeof GoldManager !== 'undefined' && typeof game !== 'undefined' && game.player) {
                console.log('\n💰 GoldManager:');
                const gmGold = GoldManager.getGold();
                const gameGold = game.player.gold;
                if (gmGold !== gameGold) {
                    console.warn(`  ❌ Gold mismatch: GM=${gmGold}, game=${gameGold}`);
                    issues++;
                } else {
                    console.log(`  ✅ Gold synced: ${gmGold}`);
                }
            }
            ...
            if (typeof EventBus !== 'undefined') {
                console.log('\n📡 EventBus:');
                const listenerCount = Object.values(EventBus._listeners || {}).reduce((sum, arr) => sum + arr.length, 0);
                const queueLength = EventBus.getQueueLength ? EventBus.getQueueLength() : 0;
                console.log(`  ✅ Listeners: ${listenerCount}`);
                console.log(`  ✅ Queued events: ${queueLength}`);
            }

            // Check SystemRegistry
            if (typeof Sys !== 'undefined') {
                console.log('\n📋 SystemRegistry:');
                const registered = Sys.list ? Sys.list().length : 'unknown';
                console.log(`  ✅ Registered systems: ${registered}`);
            }
            ...
            if (issues === 0) {
                ...
                console.log(`⚠️ FOUND ${issues} ISSUE(S) - May need manual sync`);
            }
            console.log('═══════════════════════════════════════════════════');

            return issues === 0 ? 'All systems OK!' : `${issues} issues found`;
        });

        // syncstate - Force sync all state managers
        this.registerCommand('syncstate', 'Force synchronize all state managers with game object', () => {
            console.log('🔄 Force syncing state managers...');

            // Sync GoldManager
            if (typeof GoldManager !== 'undefined' && typeof game !== 'undefined' && game.player) {
                GoldManager._gold = game.player.gold;
                GoldManager.updateAllDisplays();
                console.log('  ✅ GoldManager synced');
            }

            // Sync WorldStateManager
            if (typeof WorldStateManager !== 'undefined') {
                WorldStateManager._syncFromExistingSystems();
                console.log('  ✅ WorldStateManager synced');
            }

            // Sync PlayerStateManager
            if (typeof PlayerStateManager !== 'undefined') {
                PlayerStateManager._syncFromGame();
                console.log('  ✅ PlayerStateManager synced');
            }
        ...
        // debugstate - Show detailed state dump
        this.registerCommand('debugstate', 'Show detailed state dump of all managers', () => {
            console.log('═══════════════════════════════════════════════════');
            ...
            if (typeof PlayerStateManager !== 'undefined') {
                console.log('\n📦 PlayerStateManager:');
                console.log('  Gold:', PlayerStateManager.gold?.get());
                console.log('  Inventory items:', PlayerStateManager.inventory?.count?.() || 'N/A');
                console.log('  Equipment slots:', Object.keys(PlayerStateManager._equipment || {}).length);
            }
            ...
            if (typeof UIStateManager !== 'undefined') {
                console.log('\n🖥️ UIStateManager:');
                console.log('  Panel states:', UIStateManager._panelStates);
                console.log('  View stack:', UIStateManager._viewStack);
            }

            if (typeof Sys !== 'undefined' && Sys.getRecentChanges) {
        ...
        // trackchanges - Toggle state change tracking
        this.registerCommand('trackchanges', 'Toggle state change tracking (on/off)', (arg) => {
            if (typeof Sys === 'undefined') {
                return 'SystemRegistry not found';
            }

            if (arg === 'on') {
                Sys.enableChangeTracking();
                console.log('📝 State change tracking ENABLED');
                return 'Tracking ON';
            } else if (arg === 'off') {
                Sys.disableChangeTracking();
                console.log('📝 State change tracking DISABLED');
                return 'Tracking OFF';
            } else {
                const isTracking = Sys._trackingEnabled || false;
                console.log(`📝 State change tracking is: ${isTracking ? 'ON' : 'OFF'}`);
                console.log('Usage: trackchanges on|off');
                return isTracking ? 'Currently ON' : 'Currently OFF';
            }
        });
...
// 🔒 ONLY register if debooger is enabled in config
if (typeof GameConfig !== 'undefined' && GameConfig.debooger?.enabled === true) {
    Bootstrap.register('DeboogerCommandSystem', () => DeboogerCommandSystem.init(), {
        dependencies: ['DeboogerSystem', 'game'],
        priority: 210,
        severity: 'optional'
    });
    console.log('🎮 Debooger Command System registered with Bootstrap');
} else {
