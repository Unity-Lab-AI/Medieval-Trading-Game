...
const EnvironmentalEffectsSystem = {
    ...
    applySeasonalEffects(season) {
        ...
        if (typeof EventBus !== 'undefined' && EventBus.emit) {
            EventBus.emit('effects:season:changed', { season, config: seasonConfig });
        } else {
            document.dispatchEvent(new CustomEvent('effects:season:changed', {
    ...
    applyLocationAtmosphere(location) {
        ...
        if (typeof EventBus !== 'undefined' && EventBus.emit) {
            EventBus.emit('effects:atmosphere:changed', { location, atmosphere });
        } else {
            document.dispatchEvent(new CustomEvent('effects:atmosphere:changed', {
    ...
    applyWeatherEffects(weather, intensity) {
        ...
        if (typeof EventBus !== 'undefined' && EventBus.emit) {
            EventBus.emit('effects:weather:changed', { weather, intensity });
        } else {
            document.dispatchEvent(new CustomEvent('effects:weather:changed', {
