...
const PartyPanel = {
    ...
    healCompanion(companionId) {
        ...
        // Use item via PlayerStateManager
        if (typeof PlayerStateManager !== 'undefined') {
            PlayerStateManager.inventory.remove(healedWith, 1, 'companion_heal');
        } else {
            game.player.inventory[healedWith]--;
            const newTotal = game.player.inventory[healedWith] || 0;
            if (game.player.inventory[healedWith] <= 0) {
                delete game.player.inventory[healedWith];
            }

            // emit inventory change event (fallback)
            if (typeof EventBus !== 'undefined') {
                EventBus.emit('inventory:item:removed', {
                    itemId: healedWith,
                    quantity: 1,
                    newTotal: newTotal,
                    reason: 'companion_heal'
                });
            }
        }
...
Bootstrap.register('PartyPanel', () => PartyPanel.init(), {
    dependencies: ['game', 'PanelManager'],
    priority: 98,
    severity: 'optional'
});
