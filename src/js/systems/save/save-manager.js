...
const SaveManager = {
        ...
        return {
            ...
            gameData: {
                ...
                player: game.player ? {
                    ...
                    defeatedBosses: game.player.defeatedBosses || [],
                    bossProgress: game.player.bossProgress || {}
                } : null,
                ...
                worldState: typeof GameWorld !== 'undefined' ? {
                    ...
                    killedNPCs: GameWorld.killedNPCs || {} // NPC death tracking for respawns
                } : null,
                ...
                // ═══════════════════════════════════════════════════════════════
                //  TUTORIAL STATE - Save/load mid-tutorial progress
                //  Without this, loading a save mid-tutorial would break everything
                // ═══════════════════════════════════════════════════════════════
                tutorialState: {
                    inTutorial: game.inTutorial || false,
                    tutorialCompleted: game.tutorialCompleted || false,
                    tutorialSkipped: game.tutorialSkipped || false,
                    // Save TutorialManager progress if active
                    tutorialProgress: (typeof TutorialManager !== 'undefined' && TutorialManager.isActive)
                        ? {
                            questsCompleted: TutorialManager.tutorialProgress?.questsCompleted || [],
                            currentAct: TutorialManager.tutorialProgress?.currentAct || 0,
                            startTime: TutorialManager.tutorialProgress?.startTime || null,
                            currentQuestId: TutorialManager.currentQuestId || null
                        }
                        : null
                },
                ...
                explorationState: typeof DungeonExplorationSystem !== 'undefined' && DungeonExplorationSystem.getBossState
                    ? DungeonExplorationSystem.getBossState()
                    : null,
                // 💰 Financial tracker state - weekly totals and transaction history
                financialState: typeof FinancialTracker !== 'undefined' && FinancialTracker.getSaveData
                    ? FinancialTracker.getSaveData()
                    : null,
                // Skill system state
                skillState: typeof SkillSystem !== 'undefined' && SkillSystem.getSaveData
                    ? SkillSystem.getSaveData()
                    : null,
                // Companion system state
                companionState: typeof CompanionSystem !== 'undefined' && CompanionSystem.getSaveData
                    ? CompanionSystem.getSaveData()
                    : null,
                // Dynamic market system state - per-slot isolation (fixes localStorage cross-slot contamination)
                dynamicMarketState: typeof DynamicMarketSystem !== 'undefined' && DynamicMarketSystem.getSaveData
                    ? DynamicMarketSystem.getSaveData()
                    : null,
                // World state manager - location, world state, doom visited locations
                worldStateManagerData: typeof WorldStateManager !== 'undefined' && WorldStateManager.getSaveData
                    ? WorldStateManager.getSaveData()
                    : null
            }
        };
    },
    ...
    loadFromSlot(slotNumber) {
        ...
        try {
            ...
            // dispatch save-loaded event - achievement system was waiting for this
            // and it never fucking came. five sessions of debugging later...
            document.dispatchEvent(new CustomEvent('save-loaded', {
                detail: { slotNumber, slotName: slot.name }
            }));
            console.log('💾 save-loaded event dispatched - achievements can check tutorial status now');
    ...
    loadGameState(gameData) {
        ...
        if (gameData.player) {
            // Deep merge critical nested objects to prevent data loss
            const savedPlayer = gameData.player;
        ...
        if (gameData.worldState && typeof GameWorld !== 'undefined') {
            ...
            GameWorld.killedNPCs = gameData.worldState.killedNPCs || {}; // NPC death tracking for respawns
            GameWorld.checkNPCRespawns?.(); // Check for respawns after loading
        }

        // PRE-LOAD tutorial quest definitions BEFORE restoring active quests
        // This fixes the bug where tutorial quest IDs don't resolve on load
        if (gameData.tutorialState?.inTutorial && typeof TutorialManager !== 'undefined') {
            if (typeof TutorialManager._loadTutorialQuests === 'function') {
                TutorialManager._loadTutorialQuests();
                console.log('🎓 Pre-loaded tutorial quests before restoring active quests');
            }
        }
        ...
        if (gameData.questState && typeof QuestSystem !== 'undefined') {
            ...
            if (QuestSystem.activeQuests['act1_quest6']) {
                ...
                // Find existing objectives to preserve their progress
                const existingInnkeeperTalk = quest.objectives?.find(o => o.type === 'talk' && o.npc === 'innkeeper');
                const existingGuardTalk = quest.objectives?.find(o => o.type === 'talk' && o.npc === 'guard');

                // Build migrated objectives PRESERVING any existing completion state
                const migratedObjectives = [
                    {
                        type: 'talk',
                        npc: 'innkeeper',
                        location: 'lighthouse_inn',
                        // Preserve existing completion OR use hasJournal as fallback
                        completed: existingInnkeeperTalk?.completed || hasJournal,
                        current: existingInnkeeperTalk?.current || 0,
                    ...
                    },
                    {
                        ...
                        npc: 'guard',
                        location: 'sunhaven',
                        // Preserve existing completion state if it exists
                        completed: existingGuardTalk?.completed || false,
                        current: existingGuardTalk?.current || 0,
                        description: 'Return to Guard Captain Theron'
                    }
                ];

                // Only update if structure actually needs migration
                const needsMigration = !quest.objectives ||
                    quest.objectives.length !== 2 ||
                    !quest.objectives.some(o => o.npc === 'guard');

                if (needsMigration) {
                    quest.objectives = migratedObjectives;
                    console.log('🔧 Quest objectives migrated with preserved progress');
                } else {
                    console.log('🔧 Quest already has correct structure, skipping migration');
                }

                if (hasJournal) {
                    console.log('🔧 Player has journal, innkeeper talk marked complete');
                }
            }
        ...
        // ═══════════════════════════════════════════════════════════════
        //  TUTORIAL STATE RESTORATION - Resume mid-tutorial saves
        //  This ensures loading a save during tutorial works correctly
        // ═══════════════════════════════════════════════════════════════
        if (gameData.tutorialState) {
            // Restore game-level tutorial flags
            game.inTutorial = gameData.tutorialState.inTutorial || false;
            game.tutorialCompleted = gameData.tutorialState.tutorialCompleted || false;
            game.tutorialSkipped = gameData.tutorialState.tutorialSkipped || false;

            // If we're in tutorial mode, restore TutorialManager state
            if (game.inTutorial && typeof TutorialManager !== 'undefined') {
                console.log('🎓 Restoring tutorial state from save...');

                // Mark manager as active
                TutorialManager.isActive = true;

                // Load tutorial world if not already loaded
                if (!TutorialManager._initialized) {
                    TutorialManager.init();
                }
                TutorialManager._loadTutorialWorld();
                TutorialManager._registerTutorialNPCs();
                TutorialManager._loadTutorialQuests();

                // Restore progress if available
                if (gameData.tutorialState.tutorialProgress) {
                    TutorialManager.tutorialProgress = {
                        questsCompleted: gameData.tutorialState.tutorialProgress.questsCompleted || [],
                        currentAct: gameData.tutorialState.tutorialProgress.currentAct || 0,
                        startTime: gameData.tutorialState.tutorialProgress.startTime || Date.now(),
                        totalTime: 0
                    };
                    TutorialManager.currentQuestId = gameData.tutorialState.tutorialProgress.currentQuestId || null;
                }

                console.log('🎓 Tutorial state restored!');
            }
        }
        ...
        //  Dungeon exploration state - boss progress and defeated bosses
        if (gameData.explorationState && typeof DungeonExplorationSystem !== 'undefined' && DungeonExplorationSystem.loadBossState) {
            try {
                DungeonExplorationSystem.loadBossState(gameData.explorationState);
        ...
        // 💰 Financial tracker state - weekly totals and transaction history
        if (gameData.financialState && typeof FinancialTracker !== 'undefined' && FinancialTracker.loadSaveData) {
            try {
                FinancialTracker.loadSaveData(gameData.financialState);
                console.log('💰 Financial tracker state restored');
            } catch (e) {
                console.warn('💰 Failed to restore financial tracker state:', e.message);
            }
        }

        // Skill system state
        if (gameData.skillState && typeof SkillSystem !== 'undefined' && SkillSystem.loadSaveData) {
            try {
                SkillSystem.loadSaveData(gameData.skillState);
                console.log('🎯 Skill system state restored');
            } catch (e) {
                console.warn('🎯 Failed to restore skill system state:', e.message);
            }
        }

        // 🤝 Companion system state - hired companions
        if (gameData.companionState && typeof CompanionSystem !== 'undefined' && CompanionSystem.loadSaveData) {
            try {
                CompanionSystem.loadSaveData(gameData.companionState);
                console.log('🤝 Companion system state restored');
            } catch (e) {
                console.warn('🤝 Failed to restore companion system state:', e.message);
            }
        }

        // 💰 Dynamic market system state - per-slot market saturation/stock (fixes cross-slot contamination)
        if (gameData.dynamicMarketState && typeof DynamicMarketSystem !== 'undefined' && DynamicMarketSystem.loadSaveData) {
            try {
                DynamicMarketSystem.loadSaveData(gameData.dynamicMarketState);
                console.log('💰 Dynamic market system state restored');
            } catch (e) {
                console.warn('💰 Failed to restore dynamic market system state:', e.message);
            }
        }

        // 🗺️ World state manager - location, world state, doom visited locations
        if (gameData.worldStateManagerData && typeof WorldStateManager !== 'undefined' && WorldStateManager.loadSaveData) {
            try {
                WorldStateManager.loadSaveData(gameData.worldStateManagerData);
                console.log('🗺️ World state manager restored (doom visited locations included)');
            } catch (e) {
                console.warn('🗺️ Failed to restore world state manager:', e.message);
            }
        }
    },
    ...
    async submitToLeaderboard(gameState) {
        try {
            ...
            const scoreData = {
                ...
                characterId: player.characterId || null, // Prevents duplicate leaderboard entries
                score,
                gold: player.gold || 0,
                daysSurvived,
...
// register with Bootstrap - required for persistence
Bootstrap.register('SaveManager', () => SaveManager.init(), {
