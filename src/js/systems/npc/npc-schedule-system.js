...
const NPCScheduleSystem = {
    ...
    // Get NPC's schedule data (needed by NPCManager)
    getNPCSchedule(npcId) {
        const npcData = this.npcSchedules.get(npcId);
        if (!npcData) return null;
        return npcData.schedule;
    },
...
Bootstrap.register('NPCScheduleSystem', () => NPCScheduleSystem.init(), {
    dependencies: ['NPCManager', 'TimeSystem'],
    priority: 59,
    severity: 'optional'
});
