...
const EmployeeSystem = {
    ...
    assignEmployeeToProperty(employeeId, propertyId) {
        ...
        if (!PropertySystem.canAcceptEmployee(propertyId, employee.type)) {
            ...
            <div class="employee-assignment-interface">
                ...
                <div class="employee-current-assignment">
                    ${employee.assignedProperty ?
                        `<p>Currently assigned to: ${PropertySystem.getProperty(employee.assignedProperty)?.type || 'Unknown'}</p>` :
    ...
    populateAssignmentPropertiesList(properties, employeeId) {
        ...
        properties.forEach(property => {
            const propertyType = PropertySystem.propertyTypes[property.type];
            const isCompatible = this.isEmployeeCompatibleWithProperty(employee.type, property.type);
            ...
            propertyCard.innerHTML = `
                ...
                <div class="property-status">
                    ...
                    <div class="status-item">
                        ...
                        <span class="status-value">${property.employees.length}/${PropertySystem.getEmployeeCapacity(property.id).total}</span>
                    </div>
                </div>
    ...
    updateEmployeePerformance(employee) {
        ...
        if (employee.assignedProperty) {
            const property = PropertySystem.getProperty(employee.assignedProperty);
            if (property && this.isOptimalAssignment(employee.type, property.type)) {
                performanceScore += 15;
            }
        }
